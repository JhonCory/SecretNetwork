...
public class SecretServer {
	...
	// 'partTwo' differentiates messages for auth. part 1 and auth. part 2
	...
	private byte[] handleAuthPt1(byte[] payload) {
		...
		if (!userIdentified) {
			...
			secondHalf = SecureMethods.decryptRSA(secondHalf, my_n, my_d, simMode);
			...
			if (!Arrays.equals(Arrays.copyOf(secondHalf, supposedUser.length), supposedUser)) {
				...
				byte[] responsePayload = SecureMethods.encryptRSA(response, usersPubKeys1[userNum], BigInteger.valueOf(usersPubKeys2[userNum]), simMode);
	...
	private byte[] handleAuthPt2(byte[] payload) { 
		byte[] message = SecureMethods.decryptRSA(payload, my_n, my_d, simMode);
	...
	private boolean checkHash(byte[] password, String hashValue) {
		...
		// (Wouldn't be printed in a real-world scenario)
	...
	private byte[] processPayload(byte[] message) {
		return SecureMethods.processPayload(currentUser.getBytes(), message, counter-1, currentSessionKey, simMode);
	}


	// Takes message and packages it in "regular" payload for active session
	private byte[] preparePayload(byte[] message) {
		return SecureMethods.preparePayload("SecretServer".getBytes(), message, counter, currentSessionKey, simMode);
	}
